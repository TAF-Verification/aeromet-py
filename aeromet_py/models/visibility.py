...
class Visibility:
    ...
    def __init__(self, match: re.Match):
        if match is None:
            ...
            if match.group("vis") is not None:
                ...
                self.__visibility = _vis * CONVERSIONS.SMI_TO_KM * CONVERSIONS.KM_TO_M
            elif match.group("visextreme") is not None:
                self.__visibility = (
                    _visextreme * CONVERSIONS.SMI_TO_KM * CONVERSIONS.KM_TO_M
                )
            else:
                self.__visibility = 10_000
    ...
    def direction_in_radians(self) -> float:
        return _handle_direction(self.__direction, CONVERSIONS.DEGREES_TO_RADIANS)

    @property
    def direction_in_gradians(self) -> float:
        return _handle_direction(self.__direction, CONVERSIONS.DEGREES_TO_GRADIANS)
...
class RunwayRange:
    ...
    def __init__(self, match: re.Match):
        if match is None:
            ...
            if units == "FT":
                self.__low = float(match.group("low")) * CONVERSIONS.FT_TO_M

                try:
                    self.__high = float(match.group("high")) * CONVERSIONS.FT_TO_M
                except TypeError:
